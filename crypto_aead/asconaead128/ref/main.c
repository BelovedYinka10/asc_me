...
void init_perf() {
    ...
    if (perf_fd == -1) {
        ...
        fprintf(stderr, "Error opening performance counter. Falling back.\n");
    }
}
...
void reset_counter() {
    if (perf_fd != -1) ioctl(perf_fd, PERF_EVENT_IOC_RESET, 0);
}

void start_counter() {
    if (perf_fd != -1) ioctl(perf_fd, PERF_EVENT_IOC_ENABLE, 0);
}

void stop_counter() {
    if (perf_fd != -1) ioctl(perf_fd, PERF_EVENT_IOC_DISABLE, 0);
}
...
int main() {
    ...
    uint64_t enc_cycles = 0, dec_cycles = 0;
